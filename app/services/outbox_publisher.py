...
class OutboxPublisherService:
        ...
        async with self._session_factory() as session:
            ...
            async with session.begin():
                ...
                try:
                    ...
                    await self._retry_publish(message)
                ...
                logger.info(
                    "Published outbox event outbox_id=%s payment_id=%s event_type=%s",
                    outbox.id,
                    message.payment_id,
                    outbox.event_type,
                )
    ...
    def _build_retry_publish(
        self,
    ) -> Callable[[PaymentNewMessage], Awaitable[None]]:
        ...
        async def publish(message: PaymentNewMessage) -> None:
